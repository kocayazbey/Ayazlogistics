...
// export class MobileWmsController {
  ...
  async acknowledgeAlert(@Param('alertId') alertId: string, @Request() req: any): Promise<any> {
    return {
      success: true,
      alertId: alertId,
  ...
  async getRealTimeMetrics(@Request() req: any): Promise<any> {
    const tenantId = req.user?.tenantId;

    // Mock real-time metrics data
    const metrics = {
      activePickingOrders: Math.floor(Math.random() * 20) + 5,
      activeReceivingOrders: Math.floor(Math.random() * 15) + 3,
      activeShipments: Math.floor(Math.random() * 10) + 2,
      recentInventoryMovements: Math.floor(Math.random() * 50) + 10,
      pickingCartsInUse: Math.floor(Math.random() * 8) + 2,
      recentCompletedTasks: Math.floor(Math.random() * 30) + 5,

      equipmentUtilization: Math.floor(Math.random() * 40) + 60,
      currentThroughput: Math.floor(Math.random() * 20) + 10,
      inventoryMovements: Math.floor(Math.random() * 50) + 20
    };

    // Mock zone activity
    const zoneActivity = [
      { zone: 'A', activity: Math.floor(Math.random() * 30) + 70, operators: Math.floor(Math.random() * 5) + 3, tasks: Math.floor(Math.random() * 10) + 5 },
      { zone: 'B', activity: Math.floor(Math.random() * 30) + 70, operators: Math.floor(Math.random() * 5) + 3, tasks: Math.floor(Math.random() * 10) + 5 },
      { zone: 'C', activity: Math.floor(Math.random() * 30) + 70, operators: Math.floor(Math.random() * 5) + 3, tasks: Math.floor(Math.random() * 10) + 5 },
      { zone: 'D', activity: Math.floor(Math.random() * 30) + 70, operators: Math.floor(Math.random() * 5) + 3, tasks: Math.floor(Math.random() * 10) + 5 }
    ];

    // Mock recent events
    const recentEvents = [
      { time: new Date().toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' }), event: 'Toplama görevi tamamlandı', type: 'completion' },
      { time: new Date(Date.now() - 300000).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' }), event: 'Mal kabul işlemi başladı', type: 'start' },
      { time: new Date(Date.now() - 600000).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' }), event: 'Sevkiyat işlemi tamamlandı', type: 'completion' },
      { time: new Date(Date.now() - 900000).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' }), event: 'Envanter hareketi kaydedildi', type: 'movement' }
    ];

    return {
      timestamp: new Date(),
      liveMetrics: {
        activeOperators: Math.floor(Math.random() * 10) + 15,
        activeTasks: metrics.activePickingOrders + metrics.activeReceivingOrders + metrics.activeShipments,
        equipmentUtilization: metrics.equipmentUtilization,
        currentThroughput: metrics.currentThroughput,
        inventoryMovements: metrics.inventoryMovements
      },
      zoneActivity,
      recentEvents,
      alerts: [
        {
          id: 'alert-1',
          type: 'warning',
          message: 'Zone C\'de düşük aktivite tespit edildi',
    ...
    const orders = [
      {
        ...
        status: status || (Math.random() > 0.5 ? 'pending' : 'processing'),
        packagingType: Math.random() > 0.5 ? 'mixed' : 'uniform',
  ...
  async startItsPackaging(
    @Body() body: { orderId: string; packagingStrategy?: string },
    @Request() req: any
  ): Promise<any> {
    const tenantId = req.user?.tenantId;
  ...
  async optimizePackaging(
    @Body() body: { sessionId: string; items: any[]; constraints?: any },
    @Request() req: any
  ): Promise<any> {
    return {
      success: true,
  ...
  async createPackagingBox(
    @Body() body: { sessionId: string; boxType: string; items: any[]; tLabel?: string },
    @Request() req: any
  ): Promise<any> {
    return {
      success: true,
  ...
  async scanPackagingBox(
    @Body() body: { sessionId: string; boxId: string; barcode: string },
    @Request() req: any
  ): Promise<any> {
    return {
      success: true,
  ...
  async completeItsPackagingStep(
    @Body() body: { sessionId: string; stepId: number; data?: any },
    @Request() req: any
  ): Promise<any> {
    return {
      success: true,
  ...
  async finishItsPackaging(
    @Body() body: { sessionId: string; finalData: any },
    @Request() req: any
  ): Promise<any> {
    return {
      success: true,
